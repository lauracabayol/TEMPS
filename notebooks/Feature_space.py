...
autoencoder = Autoencoder(input_dim=10, latent_dim=2)
criterion = nn.L1Loss()
...
indexes_specz = (
    cat[(cat.specz_or_photo == 0) & (cat.reliable_S15 > 0)].reset_index().index
)
...
# First subplot
sns.kdeplot(
    x=features_all_reduced[0, indexes_nospecz, 0],
...
sns.kdeplot(
    x=features_all_reduced[0, indexes_specz, 0],
...
# Second subplot
sns.kdeplot(
    x=features_all_reduced[1, indexes_nospecz, 0],
...
sns.kdeplot(
    x=features_all_reduced[1, indexes_specz, 0],
...
# Third subplot
features_all_reduced_nospecz = (
    pd.DataFrame(features_all_reduced[2, indexes_nospecz, :]).drop_duplicates().values
)
sns.kdeplot(
    x=features_all_reduced[2, indexes_nospecz, 0],
...
sns.kdeplot(
    x=features_all_reduced[2, indexes_specz, 0],
...
legend_handles = [
    plt.Line2D([0], [0], color="salmon", lw=2),

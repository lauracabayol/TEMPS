...
from temps.utils import nmad, calculate_eta
...
photoz_archive = Archive(path_calib = parent_dir/filename_calib, 
                         ...
                         only_zspec=False)
f_test_specz, specz_test ,VIS_mag_test = photoz_archive.get_testing_data()
...
complenteness = np.arange(0,1,0.05)
for c in complenteness:
    ...
    eta_odds.append(calculate_eta(df_bin))
    if np.round(c,1) ==0.3:
        ...
        etas_odds = [calculate_eta(df[df[f'odds_{col}'] > percentile_cutoff]) for (col, percentile_cutoff) in zip(odds_test,percentiles_cutoff)]
